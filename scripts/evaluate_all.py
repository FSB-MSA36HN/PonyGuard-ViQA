...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--input", default="data/benchmark/test.jsonl"); parser.add_argument("--limit", type=int); parser.add_argument("--mock", action="store_true")
    # Mixed providers within one run are not comparable; pin one for a reportable benchmark.
    parser.add_argument("--model", help="pin one provider, e.g. 'local'")
    ...
    for system in ("basic_rag", "prompt_safe_rag", "ponyguard"):
        ...
        auditor = build_llm(config, mock=args.mock, selected_model=args.model)
    ...
    Path("reports/benchmark_metadata.json").write_text(json.dumps({"input": args.input, "sample_ids_sha256": stable_hash([row["sample_id"] for row in read_jsonl(args.input)[:args.limit]]), "pinned_model": args.model or "", "limit": args.limit, "config": load_config("configs/base.yaml")}, ensure_ascii=False, indent=2))
